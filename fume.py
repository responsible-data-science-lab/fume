...
class FairnessDebuggingUsingMachineUnlearning():
        ...
        while L <= (maxLiterals - 1):
            ...
            for subset in E:
                ...
                if subsetInfo["isValid"] == False:
                    print(subsetInfo["support"])
                    if subsetInfo["support"] > maxSupport:
                        N.append(subset)
                    else:
                        self.n_pruned += 1
                        self.pruned.append(subset)
                        continue
                ...
                if isSubsetValid:
                    ...
                    self.n_pruned += 1
                    if subset not in self.pruned:
                        self.pruned.append(subset)
            print("n_pruned: ", self.n_pruned)
            print("Len pruned: ", len(self.pruned))
            if isPruning:
                L += 1
    ...
    def __getResults(self, validSubsetsInfo):
        ...
        print("number of validSubsets: ", len(validSubsetsInfo))
        pruned = 0
        for index, subset in enumerate(validSubsetsInfo):
            ...
            if (parityReduction < 0):
                ...
                # if subset not in self.pruned:
                #     self.pruned.append(subset)
        print("pruned from last round: ", pruned)
